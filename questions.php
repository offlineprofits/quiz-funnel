...
if(isset($_POST['submit'])){
	...
	if(isset($_POST['question_type']) AND $_POST['question_type'] != ''){
		...
		if($_POST['question_type'] == 'mp' OR $_POST['question_type'] == 'ma'){
			if(isset($_POST['choice']) AND count($_POST['choice']) > 0){
				foreach($_POST['choice'] as $choice){
					if($choice != ''){
						...
						$layout->AddContentById('choices', '<input type="text" class="span4" name="choice[]" value="'.CleanToSerialize($choice).'" placeholder="{{ST:choice}}"><br/>{{ID:choices}}');
					}
				}
			...
			$layout->AddContentById('choices', '<input type="text" class="span4" name="choice[]" value="" placeholder="{{ST:choice}}"><br/>');
		}
	}else{
		$errors = true;
